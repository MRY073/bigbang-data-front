...
import {
  getNaturalStageMonitorList,
  getNaturalStageMonitorAISuggestion,
  batchGetNaturalStageMonitorAISuggestion,
  getNaturalStageMonitorChart,
  saveNaturalStageMonitorAnalysis,
...
defineOptions({ name: "NaturalStageMonitor" });
...
async function loadChartData(productId: string) {
  ...
  try {
    const result = await getNaturalStageMonitorChart({
...
const debouncedSaveAnalysis = debounce(
  async (productId: string, analysis: string, improvementPlan: string) => {
    ...
    try {
      await saveNaturalStageMonitorAnalysis({
        shopID: selectedShop.value,
        shopName: shopOption.label,
        productID: productId,
...
async function fetchData() {
  ...
  try {
    ...
    const result = await getNaturalStageMonitorList(params);
    if (result.success && result.data) {
      // 规范化数据，确保每个产品都有必要的字段
...
async function batchGetAISuggestion() {
  ...
  try {
    ...
    const result = await batchGetNaturalStageMonitorAISuggestion({
      shopID: selectedShop.value,
      shopName: shopOption.label
    });
      ...
      if (status === "new") {
        ...
        ElMessageBox.confirm(
          ...
          .then(async () => {
            ...
            const retryResult = await batchGetNaturalStageMonitorAISuggestion({
              shopID: selectedShop.value,
              shopName: shopOption.label
            });
...
<template>
  <div class="natural-stage-monitor-page">
    <div class="controls">
      <el-button
        type="primary"
